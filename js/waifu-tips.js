...
live2d_settings['homePageUrl']          = 'https://joker0ops.github.io';       // 主页地址，可选 'auto'(自动), '{URL 网址}'
...
function loadTipsMessage(result) {
    ...
    function showHitokoto() {
    	switch(live2d_settings.hitokotoAPI) {
    	    case 'hitokoto.cn':
    	        $.getJSON('https://api.hitokoto.cn/hitokoto/v1?encode=realjson',function(result){
        	        if (!empty(result.source)) {
                        var text = waifu_tips.hitokoto_api_message['hitokoto.cn'][0];
                        if (!empty(result.author)) text += waifu_tips.hitokoto_api_message['hitokoto.cn'][1];
                        text = text.render({source: result.source, creator: result.author});
                        window.setTimeout(function() {showMessage(text+waifu_tips.hitokoto_api_message['hitokoto.cn'][2], 3000, true);}, 5000);
